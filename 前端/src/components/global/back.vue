  ...
  mounted() {
    ...
    i.prototype = {
      ...
      i: function(a) {
        ...
        var b = this.b.t();
        a.strokeStyle =
          "rgba(255,255,255," + (0.02 + Math.min(0.5, b * 8.0e-4)) + ")";
    ...
    m.prototype = {
      m: function() {
        var a,
          b,
          c = 4 + Math.floor(Math.random() * 6);
        for (a = 0; a < c; ++a)
          (b = 2 + Math.random() * 8),
            a === 0 && (b += Math.random() * 15),
            (b = new l(this.x, this.y, 0.4 + Math.random() * 1.2, b)),
            (b.color = this.color),
            a > 0 &&
              (this.h.push(
                new k(
                  b,
                  this.c[Math.floor(Math.random() * a)],
                  Math.random() * 20,
      ...
      update: function() {
        ...
        for (b = this.c.length; a < b; ++a)
          (c = this.c[a]),
            (d = Math.min(d, c.a.x + c.j)),
            (e = Math.max(d, c.a.x - c.j)),
            (f = Math.min(f, c.a.y + c.j)),
            (g = Math.max(f, c.a.y - c.j)),
            c.update();
        a = 0;
        for (b = this.h.length; a < b; ++a) this.h[a].update();
        if (e < 0 || d > window.innerWidth || g < 0 || f > window.innerHeight)
          this.o = !0;
      },
      i: function(a) {
        var b, c;
    ...
    new function() {
      function a() {
        ...
        if (f)
          for (e = 0; e < n; ++e)
            (g = d[e]), g.o ? (g.s(), (d[e] = new m())) : g.update();
        else {
          b.width = b.width;
          c.globalAlpha = 0.8;
      ...
      this.m = function() {
        ...
        for (var c = 0; c < 60; ++c) d.push(new m());
